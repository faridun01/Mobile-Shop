...
try {
  ...
  const receipt = await createCashReceipt({ accountId: account.id, amount: D(100), currency: 'TJS', description: 'Audit cancel receipt', categoryName: 'Audit cancelled receipt', createdByUserId: 'user-admin' });
  ...
  const historic = await createCashReceipt({ accountId: historicAccount.id, amount: D(100), currency: 'TJS', description: 'August receipt', categoryName: 'Audit historic', createdByUserId: 'user-admin' });
  await prisma.financialTransaction.update({ where: { id: historic.id }, data: { transactionDate: new Date('2026-08-15T12:00:00Z') } });
  await createCashReceipt({ accountId: historicAccount.id, amount: D(200), currency: 'TJS', description: 'September receipt', categoryName: 'Audit historic', createdByUserId: 'user-admin' });
  ...
  const bonus = await SuppliersService.createBonus({ supplierId: 'sup-dubai', bonusType: 'CASH_DISCOUNT', amountUsd: D(100), campaignTitle: 'Audit share change', createdByUserId: 'user-admin' });
  ...
  const expense = await createExpenseStandalone({ category: 'OTHER', amountTjs: D(105), targetType: 'BUSINESS', paidFromCashRegister: false, createdByUserId: 'user-admin' });
  await shares(50);
  await deleteExpense(expense.id, 'user-admin');
  const summaryAfter = await computeReportsSummary({ period: 'ALL' });
  console.log('CANCELLED_EXPENSE_SUMMARY', JSON.stringify({ before: summaryBefore.expensesTjs, after: summaryAfter.expensesTjs, delta: D(summaryAfter.expensesTjs).minus(summaryBefore.expensesTjs), beforeOwners: beforeExpenseOwners, afterOwners: await owners() }));

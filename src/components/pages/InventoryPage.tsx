...
export const InventoryPage: React.FC = () => {
  ...
  // For admin and partner, ensure a specific warehouse/store is selected (default to main-warehouse or first store if 'all' or unset)
  const effectiveStoreId = isSeller
    ? (currentUser?.storeId || 'store-1')
    : (selectedStoreId === 'all' || !selectedStoreId ? 'main-warehouse' : selectedStoreId);

  // Filtered devices list
  const filteredDevices = useMemo(() => {
    return devices.filter((d) => {
      // 1. Role / Location Filter - strictly specific warehouse or store
      if (d.locationId !== effectiveStoreId) {
        return false;
      }
      ...
      if (selectedStatus === 'IN_STOCK') {
        ...
        if (d.purchaseCostUsd !== 0 && !d.isBonus) {
          return false;
        }

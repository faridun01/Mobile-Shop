...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden bg-[#0F1219] text-slate-100 font-mono">
      ...
      <div className="p-3 sm:p-4 border-b border-slate-800 bg-[#0B0E14] flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            Конфигурация филиалов, курса валют TJS/USD и кассовых параметров
          </p>
        </div>
      </div>
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-5 max-w-4xl">
        ...
        <div className="p-4 rounded-xl bg-[#0B0E14] border border-slate-800 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            ...
            <button
              ...
              className={`p-3.5 rounded-lg border text-left flex items-start space-x-3 transition-all ${
                theme === 'light'
                  ? 'border-blue-500 bg-emerald-500/15 ring-1 ring-blue-500/50'
                  : 'border-slate-800 bg-[#0F1219] hover:border-slate-700'
              }`}
            >
              <div className={`p-2 rounded-md ${theme === 'light' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-900 text-slate-400'}`}>
                <Sun className="w-5 h-5" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-100 uppercase">СВЕТЛЫЙ (WHITE)</span>
                  {theme === 'light' && (
                    <span className="flex items-center text-[10px] text-emerald-400 font-bold uppercase">
                      <Check className="w-3 h-3 mr-0.5" /> ВЫБРАН
                    </span>
                  )}
                </div>
                <p className="text-[11px] text-slate-400 mt-1">
                  Чистый светлый фон для дневной работы в магазине
                </p>
              </div>
            </button>
            ...
            >
              <div className={`p-2 rounded-md ${theme === 'dark' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-900 text-slate-400'}`}>
                <Moon className="w-5 h-5" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-100 uppercase">ТЁМНЫЙ (DARK)</span>
                  {theme === 'dark' && (
                    <span className="flex items-center text-[10px] text-emerald-400 font-bold uppercase">
                      <Check className="w-3 h-3 mr-0.5" /> ВЫБРАН
                    </span>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  Тёмная тема для снижения усталости глаз
                </p>
              </div>
            </button>
          </div>
        ...
        <div className="p-4 rounded-xl bg-[#0B0E14] border border-slate-800 space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-xs font-bold text-slate-950 uppercase transition-colors shadow-sm"
            >
              ИЗМЕНИТЬ КУРС
            </button>
          </div>

          <div className="p-3.5 rounded-lg bg-[#0F1219] border border-slate-800/80 flex flex-wrap items-center justify-between gap-3">
            <div>
            ...
            <div className="text-right text-xs">
              <span className="text-[10px] text-slate-500 uppercase block">ПОСЛЕДНЕЕ ОБНОВЛЕНИЕ</span>
              <span className="text-slate-300 font-bold mt-0.5 block">
                {todayRate?.date} ({todayRate?.setByName || 'Администратор'})
              </span>
            </div>
          </div>

          <p className="text-[11px] text-slate-400">
            Курс автоматически применяется при расчете розничной цены в сомони и контроле маржинальности.
          </p>
        </div>

        {/* 3. Stores & Warehouses List */}
        <div className="p-4 rounded-xl bg-[#0B0E14] border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {stores.map((s) => (
              <div key={s.id} className="p-3.5 rounded-lg bg-[#0F1219] border border-slate-800 space-y-2.5">
